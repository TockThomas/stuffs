#Update 1.1: Fuegt Gangschaltung fuer schnellere Fahrten
...
def RTrigger(shift):
    if event.type == ecodes.EV_ABS:
        if event.code == RT:
            Trigger = event.value / 12.75 * shift
            #print(Trigger)
            return Trigger
...
try:
    for event in gamepad.read_loop():
        if Lenkung() is not None:
            Lenkrad = Lenkung()
        if RTrigger(shift) is not None:
            gas = RTrigger(shift)
        if LTrigger(shift) is not None:
            brake = LTrigger(shift)
        if shift_up():
            shift = shift + 0.5
            if shift > 3:
                shift = 3
            print("Shift is on " + str(shift + shiftprint(shift)))
        if shift_down():
            shift = shift - 0.5
            if shift < 1:
                shift = 1
